module.exports = {
  ...
  custom: function(matcher, fn) {
    return function(name, schema, options, gen, depth, req) {
      var match = typeof(matcher) == 'function' ? matcher(name, schema, options, req) : name.match(matcher);
      if (match) {
        return fn(name, schema, options, req);
      }
    };
  }
};
...
function match( pattern, fn ) {
  return function ( name, schema, options, gen, depth, req ) {
    if ( schema.type === 'string' && name.match( pattern ) ) {
      return fn( options.chance, schema, req );
    }
  };
}
